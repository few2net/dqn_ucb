    ...
    def __init__(self, num_actions, death_constant, risk_constant, sess=None, multi_gpu=False, no_gpu=False):
        ...
        with tf.variable_scope('main'):
            with tf.device(device_1):
                ...
                self.d_values = linear(x, num_actions, "d_out", normalized_columns_initializer(1.0), bias_init=1.0)

                # utility value
                x = tf.nn.relu(linear(flatten(conv_out), 512, "hidden4", normalized_columns_initializer(1.0)))
        ...
        with tf.variable_scope('target'):
            with tf.device(device_2):
                ...
                # death predictor
                x = tf.nn.relu(linear(flatten(conv_out), 512, "hidden3", normalized_columns_initializer(1.0)))
                self.d_values_target = linear(x, num_actions, "d_out", normalized_columns_initializer(1.0), bias_init=1.0)
        ...
        m_tp1_best = tf.reduce_sum(tf.one_hot(tp1_best_actions, num_actions) * self.m_values_target, axis=1)
        ...
        q_var_tp1_best = tf.reduce_sum(self.q_variance_target * tf.one_hot(tp1_best_actions, num_actions), 1)
        q_var_tp1_best_masked = (1.0 - self.done_mask) * q_var_tp1_best

        u_t_selected_target = self.rewards_t + gamma * q_tp1_best_masked \
                              + c * gamma * gamma * q_var_tp1_best_masked - (b * tf.nn.relu(d_t_selected_target))
        ...
        self.optimize_expr = optimizer.minimize(errors + m_errors + d_errors + u_errors)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--b', help='death constant', type=float, default=1.0)
    ...
    experiment_name = filename + "/" + args.env + '/b_' + str(args.b) + '/c_' + str(args.c) + '/bs_' + str(args.bs) + \
                      '/seed_' + str(args.seed)
